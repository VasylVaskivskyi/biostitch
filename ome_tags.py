...
def create_ome_metadata(tag_Name, X, Y, C, Z, T, dtype, channels_meta, tag_Images, tag_MeasurementStartTime):
    ...
    pixels = pixels.format(dim_order, C, T, X, Y, Z, dtype, dtype.replace('uint' or 'int' or 'float', ''), physical_size_x, physical_size_y)

    channel = ''
    plane = ''
    IFD = 0
    for t in range(0, T):
        for c, ch_name in enumerate(channels_meta):
            channel += '{0}</Channel>'.format(channels_meta[ch_name])
            for p in range(0, Z):
                plane += '<TiffData FirstC="{0}" FirstT="{1}" FirstZ="{2}" IFD="{3}" PlaneCount="1"></TiffData>'.format(c, t, p, IFD)
                IFD += 1
